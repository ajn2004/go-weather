...
func mapHourlyResponseToHourlyForecast(resp obs.HourlyForecastResponse, number int) []weather.HourlyForecast {
	...
	for i, period := range resp.Properties.Periods {
		...
		if period.Temperature.Kind() == obs.TemperatureQV && period.Temperature.QV.Value != nil {
			...
			hf.Temperature = enforceCelsius(float64(*period.Temperature.Int), period.TemperatureUnit) // assume int is in celsius, convert to float
		}
...
func mapDailyResponseToDailyForecast(resp obs.DailyForecastResponse, number int) []weather.DailyForecast {
	// set default number to 7 days ahead
	if number <= 0 {
		number = 14 // default to 14 days as the api splits by half day
	}
	var dfs []weather.DailyForecast

	for i, period := range resp.Properties.Periods {
		if i >= number {
			break // stop if we've reached the desired number of days
		}
		var df weather.DailyForecast
		df.StartTime = period.StartTime
		df.EndTime = period.EndTime
		df.Name = period.Name
